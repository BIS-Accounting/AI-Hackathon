...
function loadRepoConfig(cwd) {
  ...
  try {
    ...
    return { teamKey: '', defaultBranch: 'main' };
  }
}
...
// The done gate: is this folder's branch committed AND pushed? Returns
// { ok, branch, upstream, remoteUrl, reason }. Never throws — a missing git
// binary reads as "not a git repo" and the caller prints the manual recipe.
function pushState(cwd) {
  if (!inGitRepo(cwd)) return { ok: false, branch: '', reason: 'not a git repo — done must run inside the ticket\'s repo or worktree' };
  const branch = git(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd }).stdout.trim() || 'HEAD';
  const dirty = git(['status', '--porcelain'], { cwd }).stdout.trim();
  if (dirty) {
    const count = dirty.split('\n').length;
    return { ok: false, branch, reason: `uncommitted changes in ${count} file(s) — commit them first` };
  }
  const up = git(['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}'], { cwd });
  if (up.code !== 0) return { ok: false, branch, reason: `branch "${branch}" has not been pushed to GitHub yet (no upstream)` };
  const upstream = up.stdout.trim();
  // best-effort refresh so "ahead" reflects the remote; offline just uses local refs
  git(['fetch', '-q'], { cwd, timeout: 10000 });
  const ahead = parseInt(git(['rev-list', '--count', '@{u}..HEAD'], { cwd }).stdout.trim(), 10) || 0;
  if (ahead > 0) return { ok: false, branch, upstream, reason: `${ahead} commit(s) on "${branch}" not pushed yet` };
  const remoteName = upstream.split('/')[0];
  const remoteUrl = git(['remote', 'get-url', remoteName], { cwd }).stdout.trim();
  return { ok: true, branch, upstream, remoteUrl };
}
...
async function verbDone(ctx) {
  ...
  const key = args._[1];
  if (!key) throw new RecipeError('done needs an issue key', ['Usage: swb done <KEY> [--link <url>] [--summary "<text>"]']);
  // The gate ALWAYS aims at the ticket's worktree when one was recorded at
  // claim time: that is where the work lives. Checking the invocation cwd let
  // a clean base repo vouch for an unpushed worktree (caught live in the first
  // user tour of the tech kit).
  ...
  out.write(`▶ push gate runs in: ${gateCwd}\n`);
  // (1) push gate — commit + push first, refuse otherwise. No network call
  // happens before this passes.
  const st = pushState(gateCwd);
  const branch = st.branch || key;
  const recipe = [
    `Save the work on the ticket's branch: git add -A, then a commit with message "${key}: <what changed>"`,
    `Push the branch to GitHub: git push -u origin ${branch}`,
    `Move ${key} to "In Review" in Linear`,
    `Post a summary comment on ${key} describing what changed (and a link to the deliverable if there is one)`,
  ];
  if (!st.ok) {
    out.write(`\n✖ ${key}: ${st.reason} — refusing to mark done.\n`);
    throw new RecipeError(`${key} is not pushed (${st.reason})`, recipe);
  }
  out.write(`  ✔ branch ${st.branch} is committed and pushed (${st.upstream})\n`);
  // (2) optional deliverable link (--link; --pr accepted as an alias)
  const link = typeof args.link === 'string' ? args.link : typeof args.pr === 'string' ? args.pr : '';
  try {
    const viewer = await getViewer(apiKey);
    const issue = await findIssueByKey(teamKey, key, apiKey);
    // (3) move In Review
    ...
    if (!summary) {
      const log = gitLogSummary(gateCwd, 5); // the branch's commits ARE the work
      summary = log ? `Recent commits:\n${log}` : 'Work completed.';
    }
    const head = `Done → In Review. Branch: ${st.branch} (pushed to ${st.remoteUrl || st.upstream})` + (link ? `\nLink: ${link}` : '');
    await postComment(issue.id, `${head}\n\n${summary}`, viewer.name, apiKey);
    // (5) remove ownership entry
    const own = readOwnership();
    if (own[key]) { delete own[key]; writeOwnership(own); }
    out.write(`✔ ${key} → In Review · branch ${st.branch} pushed${link ? ` · ${link}` : ''}\n`);
    return { code: 0 };
  } catch (err) {
    if (err instanceof RecipeError) throw err;
...
// ("Sam Lee" / @sam.lee) that digest lines and @you matching key off.
...
function usage() {
  return [
    ...
    '  swb done <KEY> [--link <url>] [--summary "<text>"]   (gate: branch committed + pushed)',
