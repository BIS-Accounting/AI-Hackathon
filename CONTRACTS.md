# Switchboard — Build Contracts (v1, accounting kit)
...
```json
{
  ...
  "issues": [ { "key": "TEA-12", "title": "...", "state": "Todo", "assignee": "name-or-null",
                "createdAt": "ISO", "updatedAt": "ISO" } ],
  "comments": [ { "issueKey": "TEA-12", "author": "name", "body": "...", "createdAt": "ISO", "discovery": false } ]
}
```
...
Repo-local: `.swb.json` → `{"teamKey": "TEA", "defaultBranch": "main"}` (`TEA2` for Team 2 – Accounting). There is no test command any more.
...
- State mapping (swb name → Linear workflow state name): IDENTITY — the kit uses Linear-native names: `Backlog, Todo, In Progress, In Review, Done` (STATE_MAP retained as doctor checklist/seam). `doctor` verifies all five exist on the team and prints which are missing (creating missing states via `workflowStateCreate` is a doctor `--fix` action).
...
swb done <KEY> [--link <url>] [--summary "<text>"]   # push gate: branch committed + pushed, else REFUSE with recipe
...
- `done` gate order: (1) push gate, run in the ticket's worktree recorded at claim time (else cwd): not a git repo / uncommitted changes / branch has no upstream / commits ahead of upstream → REFUSE with the reason and a manual recipe, before any network call; (2) optional `--link <url>` (deliverable outside the repo); (3) move In Review; (4) post summary comment (branch + remote, link if any, then `--summary` text or generated from `git log -5 --oneline`); (5) ownership entry removed. A human reviewer moves In Review → Done.
...
- LIVE round-trip tests (gated behind `SWB_LIVE_LINEAR_KEY`, team via `SWB_LIVE_TEAM_KEY` default TEA) exist for sync + show + new→claim→done, with swb-test label hygiene + teardown. Mock-only green is NOT done — the iter-1 P0s shipped precisely because mocks substring-matched buggy GraphQL.
- `swb doctor` green on this machine against team `TEA` (the designated scratch board; label-hygiene rules above are MANDATORY).
