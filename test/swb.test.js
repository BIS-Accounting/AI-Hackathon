 ...
 * done refusing on an unpushed branch, new landing in Backlog, fail-open
...
function mkRepo(cfg) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(cfg || { teamKey: 'HAC', defaultBranch: 'main' })
  );
  return dir;
}
// A real throwaway git repo for the done gate (push gate reads git state).
// withRemote → also a bare "origin" and an initial `git push -u`, so the
// branch has an upstream with nothing ahead.
function gitq(args, cwd) {
  const r = require('node:child_process').spawnSync('git', args, { cwd, encoding: 'utf8' });
  if (r.status !== 0) throw new Error(`git ${args.join(' ')} failed: ${r.stderr}`);
  return r.stdout;
}
function mkGitRepo({ withRemote = false, cfg } = {}) {
  const dir = mkRepo(cfg || { teamKey: 'HAC', defaultBranch: 'main' });
  gitq(['init', '-q', '-b', 'main'], dir);
  gitq(['config', 'user.email', 't@example.com'], dir);
  gitq(['config', 'user.name', 'Test'], dir);
  fs.writeFileSync(path.join(dir, 'work.md'), 'first draft\n');
  gitq(['add', '.'], dir);
  gitq(['commit', '-q', '-m', 'first'], dir);
  let remote = '';
  if (withRemote) {
    remote = fs.mkdtempSync(path.join(os.tmpdir(), 'swb-remote-'));
    gitq(['init', '-q', '--bare', '-b', 'main'], remote);
    gitq(['remote', 'add', 'origin', remote], dir);
    gitq(['push', '-q', '-u', 'origin', 'main'], dir);
  }
  return { dir, remote };
}
...
// Regression (live tour, 2026-07-08): the done gate must check the ticket's
// WORKTREE, not the invocation cwd — a clean, pushed base repo once vouched for
// a worktree whose branch was never pushed.
test('done: push gate runs in the recorded worktree, not the invocation cwd', async () => {
  const home = mkHome();
  const base = mkGitRepo({ withRemote: true }); // base repo: pushed + clean
  const wt = mkGitRepo();                       // stands in for the claim worktree: NEVER pushed
  await withEnv(home, 'lin_test_key', async () => {
    fs.writeFileSync(path.join(home, 'ownership.json'), JSON.stringify({
      'HAC-14': { files: ['src/*'], assignee: 'Turni', sessionId: 's1', ts: 'x', worktree: wt.dir },
    }));
    installFetch([]); // gate must refuse BEFORE any network call
    const { code, out } = await runVerb(['done', 'HAC-14'], { home, cwd: base.dir });
    assert.strictEqual(code, 2, out);
    assert.match(out, /push gate runs in: /, 'gate announces where it runs');
    assert.ok(out.includes(wt.dir), 'gate ran in the worktree, not cwd');
    assert.match(out, /not been pushed/, 'unpushed worktree branch refused the done');
  });
  rm(home); rm(base.dir); rm(base.remote); rm(wt.dir);
});
...
// ════════════════════════════════════════════════════════════════════════════
// DONE (push gate + optional --link + summary)
// ════════════════════════════════════════════════════════════════════════════

test('done: refuses outside a git repo (exit 2, recipe, no API calls)', async () => {
  const home = mkHome();
  const cwd = mkRepo({ teamKey: 'HAC', defaultBranch: 'main' }); // .swb.json only, no git
  let apiCalled = false;
  await withEnv(home, 'lin_test_key', async () => {
    ...
    assert.match(out, /MANUAL RECIPE:/);
    assert.strictEqual(apiCalled, false, 'must not touch the API when the gate fails');
  });
  rm(home); rm(cwd);
});

test('done: refuses when the branch has never been pushed (exit 2, no API calls)', async () => {
  const home = mkHome();
  const repo = mkGitRepo(); // commit exists, no upstream
  let apiCalled = false;
  await withEnv(home, 'lin_test_key', async () => {
    global.fetch = async () => { apiCalled = true; return { ok: true, status: 200, json: async () => ({ data: {} }) }; };
    const { code, out } = await runVerb(['done', 'HAC-14', '--link', 'http://sheet/1'], { home, cwd: repo.dir });
    assert.strictEqual(code, 2, out);
    assert.match(out, /not been pushed/);
    assert.match(out, /git push -u origin main/, 'recipe tells them the exact push command');
    assert.strictEqual(apiCalled, false);
  });
  rm(home); rm(repo.dir);
});

test('done: refuses with uncommitted changes (exit 2)', async () => {
  const home = mkHome();
  const repo = mkGitRepo({ withRemote: true });
  fs.writeFileSync(path.join(repo.dir, 'work.md'), 'edited but not committed\n');
  await withEnv(home, 'lin_test_key', async () => {
    installFetch([]);
    const { code, out } = await runVerb(['done', 'HAC-14'], { home, cwd: repo.dir });
    assert.strictEqual(code, 2, out);
    assert.match(out, /uncommitted changes/);
  });
  rm(home); rm(repo.dir); rm(repo.remote);
});

test('done: refuses when commits are not pushed yet (exit 2)', async () => {
  const home = mkHome();
  const repo = mkGitRepo({ withRemote: true });
  fs.writeFileSync(path.join(repo.dir, 'work.md'), 'second draft\n');
  gitq(['commit', '-q', '-am', 'second'], repo.dir);
  await withEnv(home, 'lin_test_key', async () => {
    installFetch([]);
    const { code, out } = await runVerb(['done', 'HAC-14'], { home, cwd: repo.dir });
    assert.strictEqual(code, 2, out);
    assert.match(out, /1 commit\(s\) .*not pushed/);
  });
  rm(home); rm(repo.dir); rm(repo.remote);
});

test('done: happy path (pushed branch) moves In Review, posts branch + link + summary, removes ownership (exit 0)', async () => {
  const home = mkHome();
  const repo = mkGitRepo({ withRemote: true });
  ...
  await withEnv(home, 'lin_test_key', async () => {
    ...
    const { code, out } = await runVerb(['done', 'HAC-14', '--link', 'http://sheet/9', '--summary', 'did the thing'], { home, cwd: repo.dir });
    assert.strictEqual(code, 0, out);
    assert.match(out, /HAC-14 → In Review/);
    assert.match(out, /branch main pushed/);
    assert.ok(bodies.some((b) => b.includes('http://sheet/9') && b.includes('did the thing') && b.includes('main')), 'comment has link + summary + branch');
    const own = JSON.parse(fs.readFileSync(path.join(home, 'ownership.json'), 'utf8'));
...
test('done: --link is optional; summary defaults to the branch commit log', async () => {
  const home = mkHome();
  const repo = mkGitRepo({ withRemote: true });
  const bodies = [];
  await withEnv(home, 'lin_test_key', async () => {
    ...
    const { code, out } = await runVerb(['done', 'HAC-14'], { home, cwd: repo.dir });
    assert.strictEqual(code, 0, out);
    assert.ok(bodies.some((b) => b.includes('Recent commits:') && b.includes('first')), 'summary generated from git log');
    assert.ok(!bodies.some((b) => /Link:/.test(b)), 'no Link line when --link absent');
  });
  rm(home); rm(repo.dir); rm(repo.remote);
});
...
test('refuse to run without a resolved team (exit 2)', async () => {
  const home = mkHome();
  const cwd = mkRepo({ defaultBranch: 'main' }); // no teamKey, no env
...
function liveRunner() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ teamKey: LIVE_TEAM, defaultBranch: 'main' })
  );
  const prevHome = process.env.SWITCHBOARD_HOME;
  process.env.SWITCHBOARD_HOME = home;
  return {
...
test('LIVE round-trip: sync → new(Backlog) → show → claim(In Progress) → release, then teardown', {
  ...
  try {
    ...
    // ── ask(@FirstName) → canonicalized handle; a SECOND session self-suppresses it ──
    // A caller types the viewer's FIRST name; `ask` must resolve it to the member's
    // @displayName handle. The digest side is covered by the mocked matcher tests.
    ...
    // Self-suppression is the contract: your OWN comments never echo back, even
    // when they @-mention you (a single key can only ask itself). So the fresh
    // session must see the ticket but NOT an @you line. A real @you round-trip
    // needs a second Linear user — that is what the tour's Part 2 exercises.
    assert.ok(dg.out.includes(createdKey), `digest mentions ${createdKey}: ${dg.out}`);
    assert.doesNotMatch(dg.out, /@you/, `own ask must be self-suppressed, not surfaced as @you: ${dg.out}`);
  } finally {
  ...
  try {
    await withEnv(home, null, async () => {
      ...
      const cache = {
        ...
        states: {},
        issues: [
          { key: 'HAC-1', title: 'player ui', state: 'In Progress', assignee: 'Sam Lee', createdAt: '2026-07-01T00:00:00Z', updatedAt: '2026-07-06T10:00:00Z' },
      ...
      assert.match(out, /HAC-1 {2}player ui.*→ Sam Lee/);
...
test('members: lists active team members with handle and full name', async () => {
  ...
  try {
    await withEnv(home, null, async () => {
      installFetch([
        teamMembers([
          { id: 'u1', name: 'Sam Lee', displayName: 'sam.lee', active: true },
      ...
      assert.strictEqual(code, 0, out);
      assert.match(out, /@sam\.lee\s+Sam Lee/, 'handle → full name row');
